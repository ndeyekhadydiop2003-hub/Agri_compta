...
<div class="max-w-3xl mx-auto mt-10">
    <h2 class="text-3xl font-bold text-gray-900 mb-8">Modifier une perte</h2>

    <div class="bg-white rounded-2xl shadow p-8">

        {{-- Message d'erreur du trigger --}}
        @if(session('error'))
        ...
        <form action="{{ route('pertes.update', $perte->perte_id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select name="variete_id" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">
                    <option value="">Sélectionnez une variété</option>
                    @foreach($varietes as $variete)
                        <option value="{{ $variete->variete_id }}"
                            {{ old('variete_id', $perte->variete_id) == $variete->variete_id ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="date_perte"
                       value="{{ old('date_perte', \Carbon\Carbon::parse($perte->date_perte)->format('Y-m-d')) }}"
            ...
            <div class="mb-6">
                ...
                <input type="number" step="0.01" name="quantite_kg" required
                       value="{{ old('quantite_kg', $perte->quantite_kg) }}"
            ...
            <div class="mb-6">
                ...
                <select name="motif_perte" id="motif_perte" class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                    ...
                    <option value="Pourriture" {{ old('motif_perte', $perte->motif_perte) == 'Pourriture' ? 'selected' : '' }}>Pourriture</option>
                    <option value="Insectes" {{ old('motif_perte', $perte->motif_perte) == 'Insectes' ? 'selected' : '' }}>Insectes</option>
                    <option value="Mauvais temps" {{ old('motif_perte', $perte->motif_perte) == 'Mauvais temps' ? 'selected' : '' }}>Mauvais temps</option>
                    <option value="Transport" {{ old('motif_perte', $perte->motif_perte) == 'Transport' ? 'selected' : '' }}>Transport</option>
                    <option value="Mauvaise conservation" {{ old('motif_perte', $perte->motif_perte) == 'Mauvaise conservation' ? 'selected' : '' }}>Mauvaise conservation</option>
                    <option value="Autre" {{ old('motif_perte', $perte->motif_perte) == 'Autre' ? 'selected' : '' }}>Autre</option>
                </select>
            </div>

            {{-- Motif autre --}}
            <div id="motif_autre_container" class="mt-4 {{ old('motif_perte', $perte->motif_perte) == 'Autre' ? '' : 'hidden' }}">
                ...
                <input type="text" name="motif_autre" id="motif_autre" placeholder="Ex. : Dégâts causés par les oiseaux..."
                       value="{{ old('motif_autre', $perte->motif_autre) }}"
